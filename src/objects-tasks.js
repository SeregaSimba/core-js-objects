...
function compareObjects(obj1, obj2) {
  const copyObj1 = JSON.stringify(obj1);
  const copyObj2 = JSON.stringify(obj2);

  if (copyObj1 === copyObj2) {
    return true;
  }
  return false;
}
...
function makeImmutable(obj) {
  const copyObj = Object.freeze(obj);
  return copyObj;
}
...
function makeWord(lettersObject) {
  ...
  const copyObj = Object.entries(lettersObject);

  copyObj.forEach((index) => {
    const [key, i] = index;
...
function fromJSON(proto, json) {
  ...
  const valueProto = Object.values(objParse);

  return new proto.constructor(...valueProto);
}
